    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                ...
                <p>
                    @@title@@
                    <strong><xsl:value-of select="notification_data/item_loan/title"/></strong>
                    @@due_back@@
                    <xsl:call-template name="formatDateTime">
                        <xsl:with-param name="datetime" select="notification_data/due_date"/>
                    </xsl:call-template>
                    <xsl:if test="notification_data/has_fine_value='TRUE'">
                        @@fine_policy@@
                        <xsl:choose>
                            <xsl:when test="notification_data/currency_symbol='USD'">
                                $<xsl:value-of select="notification_data/amount"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="notification_data/amount"/>&#160;<xsl:value-of select="notification_data/currency_symbol"/>
                            </xsl:otherwise>
                        </xsl:choose>
                        @@per@@
